...
Architecture:
- SSV Node "stack" in docker swarm or k8s. This repo has been tested on docker swarm; k8s would require some adjustments, though likely nothing too drastic. One copy of each service runs; on a HA deployment with multiple managers and workers, ideally across multiple AZs. The failure of one node just means those SSV services get "spun up" on another node. The node itself runs as a non-root user, UID/GID 12000.
...
- ssv.yml - the actual "stack", this would go into portainer or be deployed some other way. Things to adjust here:
...
- ssv-config.yaml - this is an example. Specify the LB FQDNs for execution (eth1) and consensus (eth2) clients, add your private key, and otherwise adjust as you see fit

Preparing the shared storage

- Have an NFS mount, create dir ssv-db
- `chown -R 12000:12000 ssv-db`

Ports accessible

- TCP 13000 and UDP 12000 need to be accessible "from Internet" so the P2P for the SSV node will work
